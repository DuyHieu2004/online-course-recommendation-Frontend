    ...
    <app-instructor-layout>
            ...
            <div class="chapter-list" *ngIf="editId" style="margin-top: 16px;">
              <div class="chapter-item card" *ngFor="let ch of chapters" style="margin-bottom: 12px; padding: 16px; background: var(--gray-50); border: 1px solid var(--gray-200);">
                <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--gray-200); padding-bottom: 12px; margin-bottom: 12px;">
                  ...
                  <button class="btn btn-sm btn-warning" style="background: #f59e0b; color: white; border: none; margin-left: 8px;" (click)="openQuizModal(ch.maChuong || ch.MaChuong)">
                      + Thêm Trắc nghiệm
                  </button>
                </div>
                <div class="lesson-list">
                  <div class="lesson-item" *ngFor="let lesson of ch.baiHocs || ch.BaiHocs" style="padding: 10px; background: var(--white); border: 1px solid var(--gray-200); border-radius: 6px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;">
                    <span style="font-size: 14px;">
                      ...
                      <!-- ========================================== -->
                      <!-- 🟢 THÊM MỚI: ĐỔI ICON NẾU LÀ BÀI TRẮC NGHIỆM -->
                      <!-- ========================================== -->
                      <i class="fa-solid fa-clipboard-question" *ngIf="isLessonQuiz(lesson.baiTap)" style="color: #f59e0b; margin-right: 6px;"></i>
                    ...
                    <!-- ========================================== -->
                    <!-- 🟢 THÊM MỚI: ẨN NÚT UPLOAD VIDEO/PDF NẾU LÀ QUIZ -->
                    <!-- ========================================== -->
                    <div style="display: flex; gap: 8px;" *ngIf="!isLessonQuiz(lesson.baiTap)">
      ...
      <div class="quiz-modal-overlay" *ngIf="showQuizModal">
        <div class="quiz-modal">
          ...
          <!-- Phần Đầu Modal (Cố định) -->
          <div style="padding: 20px 24px; border-bottom: 1px solid #eee; background: white; flex-shrink: 0;">
            <h3 style="margin: 0; color: var(--text-dark);">
              <i class="fa-solid fa-clipboard-question"></i> Tạo bài Trắc nghiệm Đánh giá
            </h3>
          </div>

          <!-- Phần Nội Dung (Cho phép cuộn dọc) -->
          <div style="flex: 1; overflow-y: auto; padding: 24px; background: #f8fafc;">
            <div class="form-group card" style="padding: 16px; margin-bottom: 20px; border-left: 4px solid #f59e0b;">
              <label style="font-size: 15px;">Điểm yêu cầu qua môn (%) *</label>
              <input type="number" class="form-control" [(ngModel)]="quizForm.passingScore" min="0" max="100" style="font-size: 16px; font-weight: bold; width: 150px;">
              <small style="color: var(--danger); font-weight: 500; display: block; margin-top: 8px;">Học viên phải trả lời đúng đạt số phần trăm này để mở khóa chương tiếp theo.</small>
            </div>

            <div class="question-block card" *ngFor="let q of quizForm.questions; let i = index" style="padding: 20px; border-radius: 8px; margin-bottom: 20px; position: relative;">
              <button class="btn btn-sm btn-icon" style="position: absolute; right: 15px; top: 15px; color: var(--danger); background: #fee2e2;" (click)="removeQuestion(i)" title="Xóa câu hỏi này">
                <i class="fa-solid fa-trash"></i>
              </button>

              <div class="form-group">
                <label style="font-size: 15px; color: var(--primary);">Câu hỏi {{ i + 1 }}</label>
                <input type="text" class="form-control" [(ngModel)]="q.text" placeholder="Nhập nội dung câu hỏi..." style="font-weight: 500;">
              </div>

              <label style="font-size: 13px; font-weight: 600; color: var(--gray-700); margin-bottom: 8px; display: block;">
                Các đáp án (Nhấn vào nút tròn để đánh dấu đáp án đúng)
              </label>

              <!-- 🟢 SỬA UI ĐÁP ÁN: LÀM NỔI BẬT ĐÁP ÁN ĐÚNG BẰNG VIỀN XANH LÁ -->
              <div *ngFor="let opt of q.options; let optIdx = index; trackBy: trackByIndex"
                   style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; padding: 10px; border-radius: 6px; transition: all 0.2s;"
                   [style.background]="q.correctIdx == optIdx ? 'rgba(16, 185, 129, 0.08)' : '#f1f5f9'"
                   [style.border]="q.correctIdx == optIdx ? '1px solid var(--success)' : '1px solid transparent'">

                <input type="radio" [name]="'correct_ans_' + i" [value]="optIdx" [(ngModel)]="q.correctIdx" title="Đánh dấu là đáp án đúng" style="transform: scale(1.3); cursor: pointer; margin-left: 5px;">

                <input type="text" class="form-control" [(ngModel)]="q.options[optIdx]" placeholder="Nội dung đáp án {{ optIdx + 1 }}" style="flex: 1; border-color: transparent;" [style.border-color]="q.correctIdx == optIdx ? 'var(--success)' : 'var(--gray-200)'">

                <!-- Huy hiệu Đáp án đúng hiển thị khi chọn -->
                <span *ngIf="q.correctIdx == optIdx" style="color: var(--success); font-size: 13px; font-weight: bold; white-space: nowrap; padding-right: 10px;">
                  <i class="fa-solid fa-check"></i> Đáp án đúng
                </span>
              </div>
            </div>

            <button class="btn btn-outline w-100" style="border-style: dashed; padding: 12px; background: white;" (click)="addQuestion()">
              <i class="fa-solid fa-plus"></i> Thêm câu hỏi
            </button>
          </div>

          <!-- Phần Chân Modal (Cố định) -->
          <div style="padding: 16px 24px; border-top: 1px solid #eee; background: white; display: flex; justify-content: flex-end; gap: 12px; flex-shrink: 0;">
            <button class="btn btn-outline" (click)="showQuizModal = false">Đóng</button>
            <button class="btn btn-primary" (click)="saveQuiz()">Lưu Bài Trắc Nghiệm</button>
          </div>
        </div>
      </div>
    </app-instructor-layout>
...
export class CreateCourseComponent implements OnInit {
  ...
  showQuizModal = false;
  currentQuizChapterId = 0;
  quizForm = {
    passingScore: 80, // Điểm qua môn mặc định
    questions: [
  ...
  openQuizModal(chapterId: number) {
    this.currentQuizChapterId = chapterId;
    this.quizForm = {
      passingScore: 80,
      questions: [{ text: '', options: ['', '', '', ''], correctIdx: 0 }]
    };
    this.showQuizModal = true;
  }

  addQuestion() {
    this.quizForm.questions.push({ text: '', options: ['', '', '', ''], correctIdx: 0 });
  }
  ...
  saveQuiz() {
    // Validate
    if (this.quizForm.questions.length === 0 || this.quizForm.questions.some(q => !q.text)) {
      Swal.fire('Lỗi', 'Vui lòng điền đủ nội dung các câu hỏi.', 'warning');
      return;
    }

    // Đóng gói JSON để lưu vào cột BaiTap của DB
    const quizData = {
      isQuiz: true,
    ...
    // Gọi API lưu Lesson (Bài học) mới, nhét JSON vào trường baiTap
    this.api.createLesson(this.currentQuizChapterId, {
      lyThuyet: 'Bài kiểm tra trắc nghiệm',
      baiTap: JSON.stringify(quizData)
    }).subscribe({
      next: () => {
        Swal.fire('Thành công', 'Đã thêm bài trắc nghiệm!', 'success');
      ...
      error: () => Swal.fire('Lỗi', 'Không thể tạo bài trắc nghiệm', 'error')
    });
  }

  // Hàm tiện ích: Kiểm tra xem Bài học này có phải là bài Trắc nghiệm không?
  isLessonQuiz(baiTapString: string | null | undefined): boolean {
    if (!baiTapString) return false;
    try {
      const data = JSON.parse(baiTapString);
      return data && data.isQuiz === true;
    } catch (e) {
      return false;
    }
  }

  // Ghi đè hàm lấy tên bài học để hiển thị cho chuẩn
  getLessonTitle(lesson: any): string {
    if (this.isLessonQuiz(lesson.baiTap)) {
      return 'Trắc nghiệm đánh giá chương';
    }
    if (lesson.lyThuyet && lesson.lyThuyet.length > 0) {
      const firstLine = lesson.lyThuyet.split('\n')[0].trim();
      return firstLine.length > 50 ? firstLine.substring(0, 50) + '...' : firstLine;
    }
    return 'Bài học #' + (lesson.maBaiHoc || lesson.MaBaiHoc);
  }
